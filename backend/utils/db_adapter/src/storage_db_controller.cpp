...
{
void StorageDbController::SetAdapter(std::shared_ptr<StorageDbAdapter> adapter)
{
    m_adapter = adapter;
}
...
{
    if (vertexFileName.empty() || rootFileName.empty() || login.empty())
    {
        return {};
    }
...
void StorageDbController::InsertNonRootFile(const std::string& fileName,
                                            const std::string& login, const std::filesystem::path& filePath) const
{
    if (fileName.empty() || filePath.empty() || login.empty())
    {
        return;
    }
...
void StorageDbController::InsertGraphArc(const std::string& login, const std::string& rootFileName,
                                         const std::string& fromFileName, const std::string& toFileName) const
{

...
DrawingView::DrawingView(QWidget* parent = nullptr)
            : m_scene{new DrawSceneModel(this)}, QGraphicsView(m_scene, parent), drawing(false)
    {
    }

    void DrawingView::mousePressEvent(QMouseEvent* event) override
    {
    ...
    void DrawingView::mouseReleaseEvent(QMouseEvent* event) override
    {
    ...
    void DrawingView::mouseMoveEvent(QMouseEvent* event) override
    {
        if (drawing)
        {
            startPoint = endPoint;
